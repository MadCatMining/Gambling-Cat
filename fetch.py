...
NODE_URL = "http://127.0.0.1:XXXX"  # Enter port number for coins rpc port
NODE_USER = "waller_user_name_goes_here"
NODE_PASSWORD = "waller_user_password_goes_here"
...
def updateBalances():
    ...
    for i in updateWallets['result']:
        cur.execute("SELECT * FROM users WHERE userid=:i",
                  {'i': i})
        getWallets = cur.fetchall()
        if getWallets != []:
            cur.execute("UPDATE users SET balance=:b WHERE userid=:i",
                      {'i': i, 'b': newBalance[i]})
            con.commit()
